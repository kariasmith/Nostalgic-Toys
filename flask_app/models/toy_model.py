...
class Toy:
    ...
    def add_toy(cls, data):
        ...
        query = "INSERT into toys (toy_name, description, year, images, created_at, updated_at, user_id) values (%(toy_name)s, %(description)s, %(year)s, %(images)s, NOW(), NOW(), %(user_id)s);"
    ...
    def edit_toy(cls, data):
        ...
        query = "UPDATE toys SET toy_name=%(toy_name)s, description=%(description)s, year=%(year)s, images=%(images)s, updated_at=NOW() WHERE id = %(id)s;"
        result = connectToMySQL(cls.DB).query_db(query, data)
        return result
